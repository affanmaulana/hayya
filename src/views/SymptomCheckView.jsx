...
export default function SymptomCheckView({ onBack }) {
  ...
  return (
    <div className="space-y-6 font-[var(--font-body)] px-1 py-4 animate-fade-in pb-10">
      ...
      <div className="w-full max-w-full overflow-hidden select-none bg-white rounded-card border border-border/80 p-4 shadow-card space-y-3">
            ...
            {childrenList.map((child) => {
              ...
              return (
                <button
                  ...
                  className={`flex-shrink-0 flex items-center gap-3 p-3 rounded-xl border text-left cursor-pointer transition-all min-w-[200px] select-none ${
                    isSelected
                      ? 'border-primary bg-primary/[0.03] shadow-sm'
                      : 'border-border bg-white hover:bg-bg/40'
                  }`}
                >
                  ...
                  <div className="min-w-0">
                    ...
                    <span className="text-xs font-extrabold text-text block truncate">{child.name}</span>
      ...
      {step === 1 && (
        <div className="space-y-5">
          ...
          <div className="bg-amber-50 border border-amber-100 rounded-card p-4 space-y-2 text-xs text-amber-900 leading-relaxed shadow-sm">
          ...
          <div className="bg-white rounded-card border border-border/80 p-5 shadow-card space-y-4">
              ...
              {[
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-between p-4 rounded-xl border text-left cursor-pointer transition-all min-h-[52px] select-none ${
                      isChecked
            ...
            {selectedSymptoms.length === 0 && (
              <p className="text-[11px] text-text-muted font-medium text-center pt-2">
            ...
            <button
              ...
              className={`w-full h-12 rounded-button text-xs font-bold transition-all shadow-md flex items-center justify-center gap-2 cursor-pointer ${
                selectedSymptoms.length > 0
      ...
      {step === 2 && (
        <div className="space-y-5">
          <div className="bg-white rounded-card border border-border/80 p-5 shadow-card space-y-5">
            <div className="space-y-1">
              <h3 className="text-sm font-extrabold font-[var(--font-heading)] text-text">
            ...
            <div className="space-y-5">
              ...
              {selectedSymptoms.includes('demam') && (
                <div className="space-y-3.5 p-4 bg-bg border border-border/80 rounded-xl">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      className="w-full h-11 px-4 text-xs rounded-xl border border-border focus:border-primary focus:outline-none bg-white font-medium"
                    />
                    {tempError ? (
                      ...
                      <p className="text-[10px] text-text-muted">Masukkan nilai suhu badan dengan angka desimal.</p>
                    )}
                  </div>
              ...
              {selectedSymptoms.includes('diare') && (
                <div className="space-y-4 p-4 bg-bg border border-border/80 rounded-xl">
              ...
              {selectedSymptoms.includes('batuk') && (
                <div className="space-y-3 p-4 bg-bg border border-border/80 rounded-xl">
            ...
            <div className="flex gap-3 pt-2">
              <button
                onClick={() => setStep(1)}
                className="flex-1 h-12 rounded-button border border-border text-text-secondary text-xs font-bold hover:bg-bg transition-all cursor-pointer"
              >
                Kembali
              </button>
              <button
                ...
                disabled={!!tempError}
                className={`flex-1 h-12 rounded-button text-xs font-bold transition-all shadow-md flex items-center justify-center gap-1.5 cursor-pointer ${
                  !tempError
                    ? 'bg-primary hover:bg-primary-dark text-white shadow-primary/10'
                    : 'bg-border text-text-muted cursor-not-allowed shadow-none'
                }`}
      ...
      {step === 3 && (
        <div className="space-y-5">
          ...
          <div className="bg-rose-50 border border-rose-100 rounded-card p-4 space-y-1.5 text-xs text-rose-950 leading-relaxed shadow-sm">
          ...
          <div className="bg-white rounded-card border border-border/80 p-5 shadow-card space-y-5">
            ...
            <div className="space-y-2.5">
              {RED_FLAGS_OPTIONS.map((flag, idx) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex gap-3.5 items-start p-3.5 rounded-xl border text-left cursor-pointer transition-all select-none ${
                      isChecked
                  ...
                  >
                    ...
                    <span className="text-[11px] font-bold text-text-secondary leading-snug">{flag}</span>
                  </button>
                );
              })}
              ...
              <button
                ...
                className={`w-full flex gap-3.5 items-center p-3.5 rounded-xl border text-left cursor-pointer transition-all select-none min-h-[50px] ${
                  hasNoRedFlags
      ...
      {step === 4 && assessmentResult && (
        <div className="space-y-6">
          ...
          {(() => {
            ...
            return (
              <div className={`rounded-card p-5 border shadow-md space-y-3 flex flex-col items-center text-center ${cfg.style}`}>
                ...
                <p className="text-xs font-medium leading-relaxed max-w-[320px] opacity-90">
                  {cfg.desc}
                </p>
              </div>
            );
          ...
          <div className="bg-white rounded-card border border-border/80 p-5 shadow-card space-y-4">
            <div className="border-b border-border/50 pb-3">
              ...
              <h3 className="text-base font-extrabold font-[var(--font-heading)] text-text leading-tight mt-0.5">
                {assessmentResult.results.diseaseName}
              </h3>
              {assessmentResult.results.medicalName && (
                <p className="text-xs text-text-secondary italic mt-0.5">({assessmentResult.results.medicalName})</p>
              )}
            </div>

            <div className="space-y-3.5 text-xs">
              <div className="space-y-1 leading-relaxed">
                <span className="text-[10px] text-text-muted font-bold uppercase tracking-wider">Tentang Penyakit:</span>
                <p className="text-text-secondary font-medium">{assessmentResult.results.description}</p>
              </div>

              {/* Home remedies / First aid guide */}
              <div className="space-y-2 pt-2 border-t border-border/40">
                <span className="text-[10px] text-text-muted font-bold uppercase tracking-wider block">🚨 Panduan Pertolongan Pertama (Home Remedies):</span>
                <div className="bg-bg rounded-xl p-4 border border-border/30 text-text-secondary font-medium leading-relaxed whitespace-normal break-words space-y-2">
                  ...
                  <div className="space-y-1.5 pl-0.5">
                    {assessmentResult.results.warningSigns.map((sign, idx) => (
                      <div key={`ws-${idx}`} className="flex gap-2.5 items-start text-rose-950 font-medium">
                        ...
                        <p className="text-[11px] leading-relaxed">{sign}</p>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          {assessmentResult.relatedArticlesFull && assessmentResult.relatedArticlesFull.length > 0 && (
            <div className="space-y-3">
              <span className="text-[10px] text-text-muted font-bold uppercase tracking-wider pl-1 block">
              ...
              <div className="grid grid-cols-1 gap-3">
                {assessmentResult.relatedArticlesFull.map((art) => {
                  ...
                  return (
                    <div
                      key={art.id}
                      className="bg-white rounded-xl border border-border/80 p-4 flex justify-between items-start gap-3 shadow-sm"
                    >
                      <div className="flex-1 min-w-0">
                        ...
                        <h4 className="text-xs font-bold text-text leading-snug mt-1.5 whitespace-normal break-words">
                          {art.title}
                        </h4>
                      </div>
                      ...
                      <button
                        ...
                        className="p-1 text-text-muted hover:text-primary transition-all focus:outline-none cursor-pointer shrink-0"
                        aria-label="Simpan artikel"
                      >
                        <svg
                          className={`w-5 h-5 ${
                            bookmarked ? 'fill-primary text-primary' : 'fill-transparent text-text-muted'
                          }`}
                          stroke="currentColor"
                          strokeWidth="2"
                          viewBox="0 0 24 24"
          ...
          <div className="p-4 bg-secondary/5 border-2 border-dashed border-secondary/20 rounded-card space-y-1.5 text-center shadow-inner">
            <span className="text-[10px] text-secondary font-black uppercase tracking-widest block">⚠️ Disclaimer Medis</span>
            <p className="text-[11px] font-semibold text-text-secondary leading-relaxed max-w-[340px] mx-auto">
          ...
          <div className="space-y-2.5 pt-2">
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 h-12 rounded-button border border-border bg-white text-text-secondary text-xs font-bold hover:bg-bg transition-all cursor-pointer flex items-center justify-center gap-2"
              >
              ...
              <button
                onClick={onBack}
                className="flex-1 h-12 rounded-button bg-primary hover:bg-primary-dark text-white text-xs font-bold shadow-md shadow-primary/10 transition-all cursor-pointer flex items-center justify-center gap-1.5"
              >
                <svg className="w-4 h-4 fill-current" viewBox="0 0 20 20">
            ...
            <button
              ...
              className="w-full h-11 rounded-button text-xs text-primary font-bold hover:bg-primary/5 transition-all cursor-pointer block border border-primary/20"
            >
